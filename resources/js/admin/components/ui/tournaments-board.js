...
function TournamentsBoard() {
  ...
  useEffect(() => {
    axios.get(ApiRoute.Tournaments['index'])
  ...
  const handleDeleteClick = (id, title) => () => {
    window.confirm(`Вы уверены что хотите безвозвратно удалить ${title}?`) &&
      axios.delete(generatePath(ApiRoute.Tournaments['delete'], { id }))
      ...
      renderCell: (params) => (
        <Stack spacing={1} direction="row">
          <IconButton
            ...
            size="small"
            href={generatePath(AppRoute.Tournaments['edit'], { id: params.row.id })}
            color="warning"
            title="Редактировать"
          >
            <EditIcon />
  ...
  const handleDeleteSelectedButtonClick = () => {
    window.confirm(
      ...
      axios.post(ApiRoute.Tournaments['multidelete'], { ids: selection })
    ...
    <>
      <Stack direction="row" justifyContent="right" marginBottom={1} spacing={1}>
        ...
        <Button
          variant="contained"
          color="success"
          href={AppRoute.Tournaments['create']}
        >
          Добавить новый
        </Button>
      </Stack>
