...
function NewsDataGrid() {
  ...
  const handleDeleteSelectedButtonClick = () => {
    ...
    isConfirmed && axios.post(APIRoute.NEWS_DELETE, { IDs: selection })
      .then(({ data }) => {
  ...
  const handleDeleteButtonClick = (id, title) => () => {
    ...
    isConfirmed &&
      axios.post(APIRoute.NEWS_DELETE, { IDs: [id] })
        .then(({ data }) => {
          toast.success(data.message);
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Stack spacing={1} direction="row" alignItems="center">
          <Button
            href={generatePath(AdminRoute.NEWS_SINGLE, { id: params.row.id })}
            variant="contained"
            color="warning"
            size="small"
          >
